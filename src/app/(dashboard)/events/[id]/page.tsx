...
import { Users, Clock, ChefHat, Calendar, Link as LinkIcon, Building2, Truck, Activity, ArrowLeft } from 'lucide-react'
...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchRealData = async () => {
      ...
      // They are linked to event_master_id (or event_id depending on how it was saved, but we'll fetch by event_id since it's the foreign key)
  ...
  if (loading) return <div className="p-20 text-center animate-pulse font-bold text-slate-400">Cargando inteligencia del evento...</div>
  if (!eventData) return <div className="p-20 text-center text-red-500 font-bold">Error: Evento no encontrado o eliminado.</div>
               ...
               <div className="space-y-4">
                  {projections.map((p, i) => (
                     <div key={i} className="flex justify-between items-center bg-slate-50 p-4 rounded-xl">
                        <span className="font-bold text-slate-800">{p.company_name}</span>
                        <span className="text-xs font-black text-indigo-500 bg-indigo-100 px-3 py-1 rounded-full">{p.projected_pax} PAX</span>
                     </div>
                  ))}
               </div>
         ...
         <div className="md:col-span-2 bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm">
            ...
            {buses.length > 0 ? (
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {buses.map((b, i) => (
                     <div key={i} className="bg-slate-50 border border-slate-100 p-5 rounded-[1.5rem]">
