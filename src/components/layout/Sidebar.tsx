    ...
    <>
      ...
      {isOpen && (
        <div 
          ...
          className="fixed inset-0 bg-slate-950/75 backdrop-blur-xs z-40 md:hidden transition-opacity duration-300 animate-in fade-in"
          aria-hidden="true"
        />
      )}

      <aside className={`fixed md:relative inset-y-0 left-0 h-screen bg-[#0f172a] border-r border-slate-800/50 flex flex-col shadow-2xl z-50 flex-shrink-0 transition-all duration-300 ease-in-out ${
        isOpen 
          ? "translate-x-0 w-72 max-w-[85vw] md:w-64 opacity-100" 
          : "-translate-x-full md:translate-x-0 md:w-0 opacity-0 pointer-events-none md:pointer-events-auto overflow-hidden"
      }`}>
        ...
        <div className="p-6 pb-4 flex-shrink-0 border-b border-slate-800/60 flex items-center justify-between">
          ...
          <button
            onClick={closeSidebar}
            className="p-2 text-slate-400 hover:text-white hover:bg-slate-800/80 rounded-xl transition cursor-pointer shrink-0 ml-1"
