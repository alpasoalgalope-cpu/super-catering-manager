...
import { createFamiliaAction } from "@/app/actions/inventory"
...
export default function FamiliaForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createFamiliaAction(nombre)
      if (result.success) {
        // Reset form
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
      <h2 className="text-sm font-black uppercase tracking-widest text-slate-400 mb-4 flex items-center gap-2">
        <Plus size={16} /> Alta de Familia
      </h2>
      <div className="space-y-2">
        <label className="text-[10px] font-bold text-slate-500 uppercase px-1">Nombre</label>
        <input 
          name="nombre"
          required
      ...
      >
        {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : "Agregar Familia"}
      </button>
    </form>
  )
}
